...
class DeformableScanRouter(nn.Module):
    """Content-adaptive spatial routing for SSM scanning.

    Standard SS4D scans along fixed raster lines. When a scan crosses an
    object boundary, the hidden state conflates semantics from two different
    regions — a structural flaw for curved or elongated targets like flood
    corridors, vegetation edges, or urban boundaries.

    Inspired by Deformable DETR: a lightweight Router predicts per-token
    spatial offsets (Δx, Δy) that warp the feature map into a
    content-adaptive layout before the SSM processes it.  The fixed-order
    scan then effectively follows object boundaries rather than raster lines.
    An inverse warp restores spatial correspondence after scanning.

    Zero-init on the offset head guarantees identity at initialisation —
    training starts from standard SSM and gradually learns deformation.

    Equivalence note: "deforming the scan path" ≡ "deforming the token
    layout then scanning in fixed order".  The latter avoids any change to
    SSM / CUDA internals and is fully differentiable via grid_sample.
    """

    def __init__(self, dim: int, max_offset: float = 0.1):
        super().__init__()
        self.max_offset = max_offset
        # DW 3×3 captures local boundary context; 1×1 projects to (Δx, Δy)
        self.router = nn.Sequential(
            nn.Conv2d(dim, dim, 3, padding=1, groups=dim, bias=False),
            nn.GELU(),
            nn.Conv2d(dim, 2, 1, bias=False),
        )
        nn.init.zeros_(self.router[-1].weight)   # identity init

    @staticmethod
    def _base_grid(B: int, H: int, W: int, device) -> torch.Tensor:
        """Normalised (x, y) coordinate grid in [-1, 1]."""
        ys = torch.linspace(-1, 1, H, device=device)
        xs = torch.linspace(-1, 1, W, device=device)
        gy, gx = torch.meshgrid(ys, xs, indexing='ij')
        return torch.stack([gx, gy], dim=-1).unsqueeze(0).expand(B, -1, -1, -1)

    def forward(self, x: torch.Tensor, ssm: nn.Module) -> torch.Tensor:
        B, C, H, W = x.shape

        # Predict offset field; scale controls max deformation magnitude
        delta = self.router(x).permute(0, 2, 3, 1) * self.max_offset  # (B,H,W,2)

        base  = self._base_grid(B, H, W, x.device)          # (B,H,W,2)
        fwd   = (base + delta).clamp(-1, 1)
        inv   = (base - delta).clamp(-1, 1)

        # Warp tokens into content-adaptive layout → SSM scans boundary-aligned sequence
        x_w = F.grid_sample(x, fwd, mode='bilinear',
                             padding_mode='border', align_corners=True)

        y_w = ssm(x_w)   # SSM sees tokens arranged along object boundaries

        # Inverse warp: restore spatial correspondence
        y   = F.grid_sample(y_w, inv, mode='bilinear',
                             padding_mode='border', align_corners=True)
        return y
...
class MBWTConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=5, stride=1, bias=True, wt_levels=1, wt_type='db1',ssm_ratio=1,forward_type="v05",):
        ...
        # SS4D: 4-direction scan with input-conditioned direction weighting
        self.global_atten = SS2D(d_model=in_channels, d_state=1,
             ssm_ratio=ssm_ratio, initialize="v2", forward_type="v05", channel_first=True,
             k_group=4, dir_weighting=True)
        # Deformable scan router: content-adaptive token layout before SSM
        self.deform_router = DeformableScanRouter(in_channels)
    ...
    def forward(self, x):
        ...
        x = self.base_scale(self.deform_router(x, self.global_atten))
        x = x + x_tag
...
def MobileMamba_T2(num_classes=1000, distillation=False, fuse=False, model_cfg=CFG_MobileMamba_T2):
    model = MobileMamba(num_classes=num_classes, distillation=distillation, **model_cfg)
    if fuse:
        replace_batchnorm(model)
    return model

def MobileMamba_T4(num_classes=1000, distillation=False, fuse=False, model_cfg=CFG_MobileMamba_T4):
    model = MobileMamba(num_classes=num_classes, distillation=distillation, **model_cfg)
    if fuse:
        replace_batchnorm(model)
    return model

def MobileMamba_S6(num_classes=1000, distillation=False, fuse=False, model_cfg=CFG_MobileMamba_S6):
    model = MobileMamba(num_classes=num_classes, distillation=distillation, **model_cfg)
    if fuse:
        replace_batchnorm(model)
    return model

def MobileMamba_B1(num_classes=1000, distillation=False, fuse=False, model_cfg=CFG_MobileMamba_B1):
    model = MobileMamba(num_classes=num_classes, distillation=distillation, **model_cfg)
    if fuse:
        replace_batchnorm(model)
    return model

def MobileMamba_B2(num_classes=1000, distillation=False, fuse=False, model_cfg=CFG_MobileMamba_B2):
    model = MobileMamba(num_classes=num_classes, distillation=distillation, **model_cfg)
    if fuse:
        replace_batchnorm(model)
    return model

def MobileMamba_B4(num_classes=1000, distillation=False, fuse=False, model_cfg=CFG_MobileMamba_B4):
    model = MobileMamba(num_classes=num_classes, distillation=distillation, **model_cfg)
    if fuse:
        replace_batchnorm(model)
    return model
...
# ── MobileMambaBackbone: 4-stage hierarchical encoder for CISRMamba ──────────
#
#  Outputs 4 channel-first feature maps compatible with VSSEncoder:
#    f1 : (B,  96, H/4,  W/4 )
#    f2 : (B, 192, H/8,  W/8 )
#    f3 : (B, 384, H/16, W/16)
#    f4 : (B, 768, H/32, W/32)
#
#  Core idea from MobileMamba:
#    - MBWTConv2d  : wavelet-domain depthwise conv + SS2D for global context
#    - MobileMambaBlock: channel-split (global / local / identity) with
#                        WaveletMamba global branch and DW-conv local branch
#  Adapted here as a dense hierarchical backbone (no classification head).

class MobileMambaBackbone(nn.Module):
    DIMS   = (96, 192, 384, 768)
    DEPTHS = (2,  2,    6,   2)
    ...
    def __init__(
        ...
        kernels: int  = 5,
        ssm_ratio: int = 1,
        forward_type: str = "v052d",
        drop_path_rate: float = 0.1,
    ):
        super().__init__()
        dims, deps = self.DIMS, self.DEPTHS
        total = sum(deps)
        dpr = [v.item() for v in torch.linspace(0, drop_path_rate, total)]

        # Stem: in_channels -> dims[0] with stride 4
        self.stem = nn.Sequential(
            Conv2d_BN(in_channels, dims[0] // 2, ks=3, stride=2, pad=1),
            nn.ReLU(),
            Conv2d_BN(dims[0] // 2, dims[0], ks=3, stride=2, pad=1),
        )
        ...
        dp = 0
        for i, (d, depth) in enumerate(zip(dims, deps)):
            stage_blocks = []
            ...
            dp += depth
            if i < len(dims) - 1:
                self.mergers.append(PatchMerging(dims[i], dims[i + 1]))
            else:
                self.mergers.append(nn.Identity())

    def forward(self, x: torch.Tensor) -> List[torch.Tensor]:
        x = self.stem(x)           # (B, 96, H/4, W/4)
        feats = []
        for i, (stage, merger) in enumerate(zip(self.stages, self.mergers)):
            x = stage(x)
            feats.append(x)
...
class MobileMambaEncoder(nn.Module):
    """
    Drop-in replacement for VSSEncoder.
    Uses MobileMamba's wavelet-SSM blocks as the hierarchical feature extractor.

    Output dims match VSSEncoder exactly:
        f1 : (B,  96, H/4,  W/4 )
        f2 : (B, 192, H/8,  W/8 )
        f3 : (B, 384, H/16, W/16)
        f4 : (B, 768, H/32, W/32)
    """
    OUT_DIMS = list(MobileMambaBackbone.DIMS)   # [96, 192, 384, 768]
    ...
    def __init__(
        ...
        super().__init__()
        self.model = MobileMambaBackbone(
