...
class SS2Dv2:
    def __initv2__(
        ...
        # SS4D: input-conditioned direction gate
        self.dir_weighting = kwargs.get('dir_weighting', False)
        if self.dir_weighting:
            self.dir_gate = nn.Sequential(
                nn.AdaptiveAvgPool2d(1),
                nn.Flatten(),
                nn.Linear(d_model, k_group),
            )
    ...
    def forward_corev2(
            ...
            # ==============================
            dir_w=None,  # (B, K) input-conditioned direction weights for SS4D
            # ==============================
            **kwargs,
    ):
        ...
        def selective_scan(u, delta, A, B, C, D=None, delta_bias=None, delta_softplus=True):
            ...
            if u.device == torch.device("cpu"):
                ...
                selective_scan_backend = None  # auto: oflex > core > mamba
        ...
        if _scan_mode == 3:
            ...
            # SS4D: apply input-conditioned direction weights before merging
            if dir_w is not None:
                ys = ys * dir_w.to(ys.dtype)[:, :, None, None, None]
    ...
    def forwardv2(self, x: torch.Tensor, **kwargs):
        # SS4D: compute input-conditioned direction weights before any projection
        dir_w = None
        if getattr(self, 'dir_weighting', False):
            dir_w = F.softmax(self.dir_gate(x), dim=1)  # (B, K)
        ...
        x = self.act(x)
        y = self.forward_core(x, dir_w=dir_w)
        y = self.out_act(y)
        y = y * z
